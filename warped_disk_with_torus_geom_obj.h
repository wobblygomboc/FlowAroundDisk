...
public:
			    ...
			    Vector<double>& dbinormal_dzeta)
 {
   ...
   // dr_x/dzeta
   dr_disk_edge_dzeta[0] = -sin(zeta_bound);

   // dr_y/dzeta
   dr_disk_edge_dzeta[1] = cos(zeta_bound);
   
   // dr_z/dzeta
   dr_disk_edge_dzeta[2] = dwdphi(r, zeta_bound);
   
   // QUEHACERES this is a temporary wrapper so we don't mess with
   // the prototype without the r_disk_edge deriv
   // -------------------------------------------------------------
   // forward to get the other derivatives
   dboundary_triad_dzeta(b, zeta_bound, dtangent_dzeta,
