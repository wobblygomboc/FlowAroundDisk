  ...
  public:
	...
	for (unsigned e=0;e<nel;e++)
	{
	  ...
	  if (count==2)
	  {
		...
		(el_pt->node_pt(1)->is_on_boundary(b)))
	    {
	      // fine
	    }
	    else
	  ...
	  {
	    std::ostringstream error_message;
	    error_message << "Boundary simplex element " << e
			  << " has " << count << " nodes on boundary "
			  << b << ", not 2!\nCoordinates: ("
			  << el_pt->node_pt(0)->x(0) << ", "
			  << el_pt->node_pt(0)->x(1) << "), ("
			  << el_pt->node_pt(1)->x(0) << ", "
			  << el_pt->node_pt(1)->x(1) << "), ("
			  << el_pt->node_pt(2)->x(0) << ", "
			  << el_pt->node_pt(2)->x(1) << ")" << std::endl;
	 
	    oomph_info << error_message.str() << std::endl;
	    /* throw OomphLibError(error_message.str(), */
	    /*                     OOMPH_CURRENT_FUNCTION, */
	    /*                     OOMPH_EXCEPTION_LOCATION); */
	  }
	...
	for (unsigned j=0;j<3;j++)
	{
      ...
      // QUEHACERES
      Tri_mesh_pt->output("disk_mesh.dat", 2);
    }
