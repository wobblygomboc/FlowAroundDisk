...
namespace oomph
{
  ...
  template <class ELEMENT>
    ...
    DiskTetMeshFacetedSurface<ELEMENT>, public virtual TetMeshFacetedClosedSurface
  {
  ...
  DiskWithTorusAroundEdgeTetMeshFacetedSurface(
    ...
    DiskTetMeshFacetedSurface<ELEMENT>(
      disk_parametrised_by_nonsingular_coordinates_pt,
      half_nsegment,
	...
	unsigned nel = this->Tri_mesh_pt->nregion_element(r);
	for(unsigned e=0; e<nel; e++)
	{
	  FiniteElement* el_pt = this->Tri_mesh_pt->region_element_pt(r,e);
	...
	//some_file.close();
      ...
      // Now loop over all elements   
      unsigned nel = this->Tri_mesh_pt->nelement();
      for (unsigned e=0; e<nel; e++)
      {
	FiniteElement* el_pt = this->Tri_mesh_pt->finite_element_pt(e); 
	...
	else
	{
      ...
      {
	unsigned nnod = this->Tri_mesh_pt->nboundary_node(b);
	for (unsigned j=0;j<nnod;j++)
	{
	  Node* nod_pt = this->Tri_mesh_pt->boundary_node_pt(b,j);
	  if (!done[nod_pt])
	  {
	    nod_pt->get_coordinates_on_boundary(b,boundary_zeta);
	    vertex_pt_and_boundary_coord.push_back(
	      std::make_pair(this->Equivalent_vertex_pt[nod_pt],
			     boundary_zeta[0]));
	    done[nod_pt]=true;
	  }
	}
      ...
      oomph_info << "Number of vertices on inner ring: "
		 ...
		 << " sum: "
		 << this->Tri_mesh_pt->nboundary_node(2) + this->Tri_mesh_pt->nboundary_node(3)
		 << std::endl;
      ...
      unsigned j=0;
      Node* nod_pt = this->Tri_mesh_pt->boundary_node_pt(b,j);
      point_in_torus[0] = this->Equivalent_vertex_pt[nod_pt]->x(0);
      point_in_torus[1] = this->Equivalent_vertex_pt[nod_pt]->x(1);
      point_in_torus[2] = this->Equivalent_vertex_pt[nod_pt]->x(2)+0.5*r_torus;
	...
	vertex_pt_and_boundary_coord.size();
      ...
      unsigned jf = this->Nfacet;
      this->Nfacet += (n_new_vertices_on_annular_ring+1)*
	(vertex_pt_and_boundary_coord.size());
      Facet_pt.resize(this->Nfacet);
  ...
  DiskWithTwoLayersTetMeshFacetedSurface(
    ...
    DiskTetMeshFacetedSurface<ELEMENT>(disk_parametrised_by_nonsingular_coordinates_pt,
				       half_nsegment,
				       first_one_based_boundary_id_for_disk,
				       last_one_based_boundary_id_for_disk)
    {
	...
	Facet_pt[e]->set_one_based_adjacent_region_id(
      ...
      // side and two lids.
      this->Nfacet += 2* this->Nelement_on_disk_boundary+2;
      Facet_pt.resize(this->Nfacet);
      this->Facet_is_on_boundary.resize(this->Nfacet,false);
      ...
      for (unsigned e=0;e<nel;e++)
      {
	if (this->Facet_is_on_boundary[e])
	{
